...
stock Motor_PMSM_dual (ModelExchange) -> GeorgeEMS_Knee_v11 (CoSimulation),
...
FMU = r"C:\Users\George\OneDrive\Desktop\PhD Thesis\Simulink EMS\GeorgeEMS_Knee_v11.fmu"
BAT = r"C:\Program Files\Altair\2025\hwsolvers\scripts\motionsolve.bat"
RUN = os.path.join(os.environ["TEMP"], "knee_ms_run")

os.makedirs(RUN, exist_ok=True)
for f in (DECK, "custom_event_tipout_10.adf"):
    shutil.copy(os.path.join(SRC, f), RUN)
shutil.copy(FMU, os.path.join(RUN, "GeorgeEMS_Knee_v11.fmu"))
...
#    Motor_PMSM_dual.fmu)
new_fmu = os.path.join(RUN, "GeorgeEMS_Knee_v11.fmu").replace("\\", "/")
