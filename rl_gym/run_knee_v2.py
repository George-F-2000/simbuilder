...
stock Motor_PMSM_dual (ModelExchange) -> GeorgeEMS_Knee_v2 (CoSimulation),
...
RUN = os.path.join(os.environ["TEMP"], "knee_ms_run_v2")

os.makedirs(RUN, exist_ok=True)
for f in (DECK, "custom_event_tipout_10.adf"):
    shutil.copy(os.path.join(SRC, f), RUN)
shutil.copy(FMU, os.path.join(RUN, "GeorgeEMS_Knee_v2.fmu"))
...
#    Motor_PMSM_dual.fmu)
new_fmu = os.path.join(RUN, "GeorgeEMS_Knee_v2.fmu").replace("\\", "/")
